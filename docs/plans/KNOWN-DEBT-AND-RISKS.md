...
| Plan | 类别 | 描述 | 参考 |
|---|---|---|---|
| 019 | 已修复 | ~~`load_builtin` 返回 `Option<Box<dyn Role>>`，rust-ref 用 `Option<Arc<dyn Role>>`。转译版 `resolve_role` 因此返回 `Option<Arc<Box<dyn Role>>>`（双层包装）。编译/功能可用（Deref 链），但精度有损~~ — auto-lang **Plan 390 §15.11（L2 转正）已交付**：a2r 对 Box/Arc 容器内的 spec 统一渲染 `dyn`（`Arc<Role>` → `Arc<dyn Role>`），值侧 spec-bound 实参 `Arc(role)` → `Arc::from(role)`，双层包装消除、与 rust-ref 对齐。**待验证**：auto-ai retranspile 后 `resolve_role` 应渲染为 `Option<Arc<dyn Role>>`。 | `crates/auto-ai-agent/rust/src/builtin_roles.rs:23` |
