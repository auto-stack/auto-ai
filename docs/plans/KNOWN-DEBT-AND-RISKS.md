...
| 021 | 已知限制 | Auto/a2r **缺少闭包类型**（`Box<dyn Fn>`/`impl Fn`/`Arc<dyn Fn>`）。EventSink cb 是裸 `fn(StreamEvent)` 指针，能持不捕获的 fn（noop_event），但不能持捕获了 `on_event` 的闭包。阻塞 driver Delta/Tool 转发（Plan 021 Phase 6.6）。`fn(params){}` 参数绑定已修（Plan 390 Phase H，f40b404c），但闭包→fn 指针的类型不匹配仍阻。rust-ref 用 `Arc<dyn Fn>`，Auto 无法表达。driver 非流式事件已正常工作，仅流式 Delta/Tool 受阻。需 Auto 增加闭包类型表达。 | `crates/auto-ai-agent/src/orchestration/driver.at:303`（drive_step，run 非流式）|
