...
| 021 | 已知限制 | Auto 闭包**不能捕获外部变量**（`fn(ev) { forward(ev, outer_cb) }` 报 "Variable ev not defined"）。阻塞 Plan 021 Phase 6.6：driver 的 Delta/Tool → PipelineEvent 转发需 EventSink 的 `cb fn(StreamEvent)` 捕获 `on_event`，但 cb 只能接收单参数（事件），EventSink actor 又无法持 `on_event` 引用。rust-ref 用 `Arc<dyn Fn>` 闭包捕获，Auto 无等价。需 Auto 支持 `Arc<dyn Fn>` 闭包捕获（语言级）或非 actor 流式架构。driver 的非流式事件（StepStarted/Completed/Failed/Paused/GateWaiting/BudgetWarning）已正常工作，仅流式 Delta/Tool 受阻。 | `crates/auto-ai-agent/src/orchestration/driver.at:303`（drive_step，run 非流式）|
