...
| 021 | 已知限制 | ~~Auto 闘包不能捕获外部变量~~ → **重新定性（2026-08-07 实证）**：捕获正常（`(ev) => fwd(ev, outer_cb)` 工作）。真因是 `fn(params){}` 解析路径（`parser.rs:3043-3069`）不 bind 闭包参数（另两条路径 `x =>` / `(a,b) =>` 都 bind）。阻塞 driver Delta/Tool 转发（Plan 021 Phase 6.6）。修复是 ~6 行 parser 改动（auto-lang Plan 390 §15 Phase H）。driver 非流式事件已正常工作，仅流式 Delta/Tool 待 Phase H 解锁。 | `crates/auto-ai-agent/src/orchestration/driver.at:303`（drive_step，run 非流式）|
