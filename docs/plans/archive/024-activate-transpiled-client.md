...
> 2026-08-23 复审回填：以下 checkbox 当初未随实施记录回填（文档不一致），现按 2026-08-07 实施记录全部核对勾选。

### Phase 1 — 修 .at 源码 visibility + 签名（auto-ai-client）
- [x] 1.1 `src/lib.at`：`type AiClient` → `pub type AiClient`
- [x] 1.2 `ext AiClient` → `pub ext AiClient`
- [x] 1.3 `new`/`with_url`/`default`/`url`/`is_daemon_mode` 加 `pub`
- [x] 1.4 `complete_stream` 的 `on_event fn(JsonValue)` → `on_event Arc<Fn(JsonValue)>`
- [x] 1.5 retranspile `crates/auto-ai-client/retranspile.sh`，确认 `rust/src/lib.rs` AiClient 变 pub、
      complete_stream 签名含 `Arc<dyn Fn(JsonValue) + Send + Sync>`

### Phase 2 — 依赖切换（agent 转译树指向转译版 client）
- [x] 2.1 `auto-ai-agent/rust/Cargo.toml`：`auto-ai-client = { path = "../../auto-ai-client" }`
      → 改为指向转译版 client crate
- [x] 2.2 **关键决策**：转译版 client crate 名是 `auto-ai-client-a2r`（lib 名 `auto_ai_client_a2r`），
...
- [x] 2.3 重新 build `auto-ai-agent/rust/`，确认解析到转译版 client

### Phase 3 — client_impl.rs 适配（impl Client for 转译版 AiClient）
- [x] 3.1 `client_impl.rs` 当前 `impl Client for AiClient`（rust-ref 的 AiClient）。
...
- [x] 3.2 确认：转译版 AiClient 是否能直接 `impl Client for AiClient`（在 client_impl.rs 胶水里）。
      需要 AiClient 是 pub（Phase 1 已修）。impl 体调 AiClient 自己的 complete/complete_stream。
- [x] 3.3 StreamingAiClient 的处理：它包装真实 reqwest AiClient 做 SSE 侧信道。
      如果改用转译版 client，侧信道是否还需要？转译版 client 的 complete_stream 已用 a2r-std http
      做 SSE。StreamingAiClient 可能可以删除或改造。

### Phase 4 — 验证
- [x] 4.1 `auto-ai-agent/rust/` cargo check 0 错
- [x] 4.2 `auto-ai-react` 二进制重新 build
- [x] 4.3 **e2e 实跑**（daemon 在运行 + 配置文件有 key）：确认转译版 client 经 a2r-std http 能与 daemon 通信
- [x] 4.4 转译版测试套件（transpiled_harness 15 测试）仍全绿（mock 不受影响——它们不依赖 AiClient）
- [x] 4.5 workspace 回归（rust-ref client 不受影响）

### Phase 5 — KNOWN-DEBT 记录
- [x] 5.1 记录 sync-in-async（a2r-std http 用 ureq 同步，async fn 实际阻塞）作为已知限制
- [x] 5.2 记录转译版 client 与 rust-ref 的行为差异（如有）
