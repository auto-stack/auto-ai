...
fn load_from_env() -> DaemonConfig {
    ...
    if let Ok(key) = std::env::var("ZHIPU_API_KEY") {
        providers.insert(
            "zhipu".into(),
            provider_env("openai", "https://open.bigmodel.cn/api/paas/v4", key, vec!["glm-4.5".into(), "glm-4-flash".into()]),
        );
    }
    if let Ok(key) = std::env::var("ANTHROPIC_API_KEY").or_else(|_| std::env::var("ANTHROPIC_AUTH_TOKEN")) {
        providers.insert(
            "anthropic".into(),
            provider_env("anthropic", &std::env::var("ANTHROPIC_BASE_URL").unwrap_or_else(|_| "https://api.anthropic.com".into()), key, vec!["claude-3-5-sonnet-20241022".into()]),
        );
    }
    if let Ok(key) = std::env::var("OPENAI_API_KEY") {
        providers.insert(
            "openai".into(),
            provider_env("openai", &std::env::var("OPENAI_BASE_URL").unwrap_or_else(|_| "https://api.openai.com/v1".into()), key, vec!["gpt-4o".into()]),
        );
    }
...
fn provider_env(kind: &str, base_url: &str, key: String, models: Vec<String>) -> ai_config::ProviderConfig {
